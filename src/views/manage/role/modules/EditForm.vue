...
export default {
  ...
  methods: {
    ...
    setFieldsValue () {
      ...
      $nextTick(() => {
        setFieldsValue(_.pick(this.record, ['role_name', 'parent_id', 'sort']))
      })
    },
    ...
    // 格式化角色列表
    formatTreeForRoleList (list, disabled = false) {
      const data = []
      list.forEach(item => {
        // 新的元素
        const newItem = {
        ...
        // 禁用的角色
        if (
          [item.role_id, item.parent_id].includes(this.record.role_id) ||
          disabled === true
        ) {
        ...
        // 递归整理子集
        if (item.children && item.children.length) {
          newItem['children'] = this.formatTreeForRoleList(item['children'], newItem.disabled)
        }
        data.push(newItem)
      })
      return data
    ...
    formatTreeDataForMenuList (list) {
      ...
      list.forEach(item => {
        ...
        // 递归整理子集
        if (item.children && item.children.length) {
          newItem['children'] = this.formatTreeDataForMenuList(item['children'])
        }
        data.push(newItem)
      })
      return data
    ...
    onFormSubmit (values) {
      ...
      Api.edit({ roleId: this.record['role_id'], form: values })
        .then(result => {
          // 显示成功
